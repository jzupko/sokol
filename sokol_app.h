...
_SOKOL_PRIVATE void _sapp_x11_init_extensions(void) {
    ...
    _sapp.x11.SAPP_SELECTION          = XInternAtom(_sapp.x11.display, "SAPP_SELECTION", False);
...
_SOKOL_PRIVATE const char* _sa_sapp_x11_get_clipboard_string(void) {
    ...
    sevt = &notification.xselection;
    if (sevt->property == None || _sapp.x11.CLIPBOARD != sevt->selection || _sapp.x11.SAPP_SELECTION != sevt->property) {
        return NULL;
    }

    ret = (Success == XGetWindowProperty(_sapp.x11.display,
                        sevt->requestor,
                        sevt->property,
                        0,
                        (long)_sapp.clipboard.buf_size,
                        False,
                        AnyPropertyType,
                        &actualType,
                        &actualFormat,
                        ...
                        &bytesAfter,
                        (unsigned char**) &data));
    if (!ret || data == NULL || 0 == actualFormat || actualType != _sapp.x11.UTF8_STRING) {
        if (data != NULL)
            XFree(data);
        return NULL;
    }
...
_SOKOL_PRIVATE void _sapp_x11_process_event(XEvent* event) {
    ...
    switch (event->type) {
        ...
        case SelectionRequest: {
            ...
            if (req->target == _sapp.x11.TARGETS) {
                Atom supported_targets[] = { _sapp.x11.TARGETS, _sapp.x11.UTF8_STRING };
                XChangeProperty(req->display,
                                ...
                                32,
                                PropModeReplace,
                                (unsigned char*)&supported_targets[0],
                                sizeof(supported_targets)/sizeof(supported_targets[0]));
            // Otherwise, respond with data only for utf8 request and the clipboard selection.
            } else if (req->target == _sapp.x11.UTF8_STRING && req->selection == _sapp.x11.CLIPBOARD) {
                XChangeProperty(req->display,
                                req->requestor,
                                req->property,
                                _sapp.x11.UTF8_STRING,
